...
def insert_jobs(jobs):
    ...
    # We also find today's date to keep track of when we crawled the job
    ...
    for i in jobs:
        ...
        VALUES (%s, %s, %s, %s, %s, %s, %s) ON CONFLICT (job_id) DO NOTHING""", (title, job_id, company_name, jd_url, jd, crawled_id, crawled_on,))
    ...
    cursor.execute(
        "insert into crawl_status (crawled_on) values (%s)", (crawled_on,))
    dbconn.commit()


# If we run "flask crawl", this function will run.
