...
def alljobs():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "select o.id, o.title, o.company_name, s.name from openings o, job_status s where s.id = o.status order by s.name")  # Query
    ...
    # Render the data using the jobs/jobslist template.
...
def edit_job(jid):
    ...
    if request.method == "GET":
        ...
        cursor.execute(
            "update openings set jd_text = %s, status=%s where id=%s", (jd, status, jid))
        conn.commit()
